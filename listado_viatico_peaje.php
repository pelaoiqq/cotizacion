...
$limit = 20; // Número de cotizaciones por página //para paginacion
...
if (!empty($fecha_inicio) && !empty($fecha_fin)) {
    $conditions[] = "ct.fecha_inicio BETWEEN '$fecha_inicio' AND '$fecha_fin'"; // Filtro por fecha_inicio
}

// Agregar condiciones a la consulta si existen filtros
if (count($conditions) > 0) {
    $sql .= " AND " . implode(" AND ", $conditions);
}

// Agrupar por id_cotizacion y fecha_inicio
$sql .= " GROUP BY ct.id_cotizacion, ct.fecha_inicio ORDER BY ct.fecha_inicio DESC, ct.id_cotizacion DESC";

// Obtener total de resultados para paginación //para paginacion
$total_sql = "SELECT COUNT(*) as total FROM (" . $sql . ") AS subquery"; //para paginacion
$total_result = $conn->query($total_sql); //para paginacion
$total_rows = $total_result->fetch_assoc()['total']; //para paginacion
$total_pages = ceil($total_rows / $limit); //para paginacion

$sql .= " LIMIT $limit OFFSET $offset"; //para paginacion
...
<div class="container my-5">
    <h2 class="text-center mb-4">Listado de Viaticos y Peajes</h2>

    <form method="GET" action="listado_viatico_peaje.php" class="mb-4">
        <div class="row g-3">
            <div class="col-md-3">
            ...
            <div class="col-md-3">
                ...
                <a href="listado_viatico_peaje.php" class="btn btn-secondary">Limpiar</a>
            </div>
        </div>
    </form>

    <!-- Aquí agregas el botón CSV -->
    <form method="GET" action="exportar_viatico.php">
        ...
        <table class="table table-bordered table-striped mt-3">
            ...
            <tbody>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <?php 
                        $total_fila = $row['total_viatico'] + $row['total_peaje'];
                        $gran_total += $total_fila;
                    ?>
                    <tr>
                        <td><?php echo $row['id_cotizacion']; ?></td>
                        <td><?php echo date("d-m-Y", strtotime($row['fecha_inicio']));?></td>
                        <td class="text-end">$<?php echo number_format($row['total_viatico'], 0, ',', '.'); ?></td>
                        <td class="text-end">$<?php echo number_format($row['total_peaje'], 0, ',', '.'); ?></td>
                        <td class="text-end">$<?php echo number_format($total_fila, 0, ',', '.'); ?></td>
                    </tr>
                    <?php
                    $total_viatico += $row['total_viatico'];
                    $total_peaje += $row['total_peaje'];
                    ?>
                <?php endwhile; ?>
            </tbody>
            <tfoot>
                <tr class="table-secondary">
                    ...
                    <th class="text-end">$<?php echo number_format($total_viatico, 0, ',', '.'); ?></th>
                    <th class="text-end">$<?php echo number_format($total_peaje, 0, ',', '.'); ?></th>

...
header('Content-Disposition: attachment; filename="listado_neumatico_lavado_' . date('Ymd_His') . '.csv"');

// Abrir salida
$output = fopen('php://output', 'w');

// Encabezado de columnas
fputcsv($output, ['Cotización', 'Fecha Inicio', 'Lavado de Camión', 'Reparación Neumáticos', 'Total']);
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        $total = $row['total_lavado'] + $row['total_neumatico'];
        fputcsv($output, [
            $row['id_cotizacion'],
            date("d-m-Y", strtotime($row['fecha_inicio'])),
            number_format($row['total_lavado'], 0, ',', '.'),
            number_format($row['total_neumatico'], 0, ',', '.'),
